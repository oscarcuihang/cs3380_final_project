...
<div id="main_content">	
		...
		<?php
			...
			$query = 'SELECT * FROM final_project.user_infor WHERE username = $1';
		...
		<p id="text">User Description</p> <textarea name='description' rows='20' cols='60'>Enter new description here</textarea><br/>
			<input id = "input_box" type="submit" value="Update" name="update"><br/>
			<input id = "input_box" type="button" value="Cancel" onclick="top.location.href = 'my_page.php'">
		</form>
		<?php
			...
			/*if (isset($_POST['update']))
			{
				...
				{	
					...
					//if password updated
					if ( $_POST['password'] != ''){
						$queryp = "UPDATE finl_project.user_authentication SET
								salt = $1,
								password_hash = $2,
								WHERE username = $3";
					...
					$query1 = "UPDATE final_project.user_infor SET 
								email = $1,
								phone = $2,
								description = $3 
								WHERE username = $4";
			...
			if (isset($_POST['update']))
			{
				if (( $_POST['password'] != $_POST['confirm_password']) || $_POST['password']=='' || $_POST['confirm_password']=='' || $_POST['email'] == '')
				{
					echo "<p id = 'text'>Invalid input, please try again</p>";
				}
				else 
